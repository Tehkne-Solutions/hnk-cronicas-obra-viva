...
function appendEvent(chronicle: ChronicleSaveV2, type: string, payload: Record<string, unknown>): ChronicleSaveV2 {
  ...
  return { ...chronicle, eventLedger: [...chronicle.eventLedger, { id, type, occurredAt: chronicle.world.timestamp, payload }] };
}
...
function App() {
  ...
  async function refreshSessions() {
    try { setSummaries(await listChroniclesFromBrowser()); setStorageState("ready"); }
    catch { setSummaries([]); setStorageState("error"); }
  }
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      ...
      void saveChronicleToBrowser(chronicle).then(() => setStorageState("ready")).catch(() => setStorageState("error"));
    }, 150);
  ...
  async function continueChronicle(id: string) {
    ...
    if (!saved) return;
    setChronicle(saved); setSessionState("playing");
  }
  ...
  if (sessionState === "menu" || !chronicle) return (
    <main className="shell session">
      <p className="eyebrow">HENUVOKODAN</p><h1>Crônicas da Obra Viva</h1>
      <section className="session-card">
        <h2>Escolha sua Crônica</h2>
        <button className="primary" onClick={newChronicle}>Nova Crônica</button>
        {summaries.length === 0 ? <p className="note">Nenhuma Crônica preservada neste navegador.</p> : summaries.map((item) => (
  ...
  function travel(to: LocationId) { const key = `${location}>${to}`; const reverse = `${to}>${location}`; const minutes = travelMinutes[key] ?? travelMinutes[reverse] ?? 15; const result = travelChronicle(chronicle, personaId, { from: location, to, travelMinutes: minutes }); setChronicle(appendEvent(result.chronicle, "Travelled", { from: location, to, minutes })); }
  ...
  return <main className="shell"><header><div><p className="eyebrow">HENUVOKODAN</p><h1>Crônicas da Obra Viva</h1></div><div><div className="clock">{formatTime(now.day, now.minuteOfDay)}</div><p className="note" aria-live="polite">{storageState === "saving" ? "Salvando Crônica…" : storageState === "error" ? "Persistência local indisponível" : "Crônica preservada"}</p></div></header>
    <section className="book"><article className="page narrative"><p className="location">{labelFor(location)}</p><p style={{ whiteSpace: "pre-line" }}>{narrativeText}</p><div className="actions"><button onClick={observe}>Observar</button><button onClick={wait}>Esperar 15 min</button><button onClick={() => setLiberOpen((value) => !value)}>LIBER</button><button onClick={() => void returnToMenu()}>Voltar ao início</button></div></article><aside className="page routes"><h2>Para onde ir?</h2>{nearby.map(([id, label]) => <button key={id} onClick={() => travel(id)}>{label}</button>)}<p className="note">Viajar consome tempo. Pessoas, locais e consequências podem mudar enquanto você está a caminho.</p></aside></section>
    {liberOpen && <section className="liber"><div><strong>DIARIUM</strong><span>{chronicle.eventLedger.length} eventos preservados</span></div><div><strong>MATERIA</strong><span>{liber.materiaNodeIds.length} materiais reconhecidos</span></div><div><strong>QUAESTIONES</strong><span>{liber.questionIds.length} questões registradas</span></div><div><strong>EXPERIMENTA</strong><span>{liber.experiments.length} experimentos reconstruídos</span></div></section>}<footer>Tehkné Solutions</footer></main>;
}
