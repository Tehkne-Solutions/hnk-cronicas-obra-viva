...
function eventTime(event: StoredTelemetryEvent): string { return event.receivedAt || event.occurredAt; }
function millisBetween(start: string, end: string): number { return Math.max(0, Date.parse(end) - Date.parse(start)); }
function transition(state: IncidentLifecycleState, at: string, event: StoredTelemetryEvent): IncidentLifecycleTransition {
...
export function deriveIncidentLifecycles(events: readonly StoredTelemetryEvent[], nowIso: string): readonly IncidentLifecycle[] {
  ...
  for (const event of regressions) {
    ...
    else { current.regressionEvents.push(event); if (intelligence.lastSeenAt > current.intelligence.lastSeenAt) current.intelligence = intelligence; }
  }
  const result: IncidentLifecycle[] = [];
  for (const [fingerprint, group] of byFingerprint.entries()) {
    const related = events.filter((event) => eventFingerprint(event) === fingerprint && TRANSITIONS[event.name]).sort((a, b) => eventTime(a).localeCompare(eventTime(b)));
    const regressionsForFingerprint = [...group.regressionEvents].sort((a, b) => eventTime(a).localeCompare(eventTime(b)));
    const first = regressionsForFingerprint[0];
    if (!first) continue;
    const openedAt = eventTime(first);
    ...
    let reopenCount = 0;
    const transitions: IncidentLifecycleTransition[] = [transition("open", openedAt, first)];
    const timeline = [...related.map((event) => ({ kind: "transition" as const, event, at: eventTime(event) })), ...regressionsForFingerprint.slice(1).map((event) => ({ kind: "regression" as const, event, at: eventTime(event) }))].sort((a, b) => a.at.localeCompare(b.at));
    for (const entry of timeline) {
      if (entry.kind === "regression") {
        if (state === "resolved") {
          state = "open"; reopenCount += 1; investigationStartedAt = undefined; mitigatedAt = undefined; resolvedAt = undefined; resolvedBuildSha = undefined;
          transitions.push(transition("open", entry.at, entry.event));
        }
        continue;
      }
      ...
      if (next === "resolved") { resolvedAt = entry.at; const value = entry.event.data?.resolvedBuildSha; resolvedBuildSha = typeof value === "string" ? value : entry.event.buildSha; }
      state = next; transitions.push(transition(next, entry.at, entry.event));
    }
    const terminalAt = resolvedAt ?? nowIso;
    result.push(Object.freeze({ ...group.intelligence, lifecycleState: state, openedAt, ...(investigationStartedAt ? { investigationStartedAt } : {}), ...(mitigatedAt ? { mitigatedAt } : {}), ...(resolvedAt ? { resolvedAt } : {}), ...(resolvedBuildSha ? { resolvedBuildSha } : {}), openDurationMs: millisBetween(openedAt, terminalAt), ...(investigationStartedAt ? { mttaMs: millisBetween(openedAt, investigationStartedAt) } : {}), ...(resolvedAt ? { mttrMs: millisBetween(openedAt, resolvedAt) } : {}), reopenCount, transitions: Object.freeze(transitions) }));
  }
