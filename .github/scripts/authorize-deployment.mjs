...
const environment = process.env.HNK_DEPLOYMENT_ENVIRONMENT ?? "production";
...
if (candidate) {
  ...
  if (candidate.evidence?.controlCenterStorage === "memory" || !candidate.evidence?.controlCenterStorage) failures.push(`candidate_storage_invalid:${candidate.evidence?.controlCenterStorage ?? "missing"}`);
...
if (!baseUrl || !adminToken) {
  failures.push("control_center_not_configured");
} else {
  try {
    health = await jsonFetch("/health");
    if (health?.ok !== true) failures.push("control_center_health_not_ok");
    if (health?.storage === "memory") failures.push("control_center_not_persistent");
    snapshot = await jsonFetch("/api/snapshot?hours=6", { headers: { authorization: `Bearer ${adminToken}` } });
  } catch (error) {
    failures.push(`control_center_unreachable:${error instanceof Error ? error.message : String(error)}`);
  }
}
...
const critical = Array.isArray(snapshot?.diagnostics)
...
const freshFatals = Array.isArray(snapshot?.recentEvents)
  ? snapshot.recentEvents.filter((event) => event?.level === "fatal" && !ignoredSession(event?.sessionId))
  : [];
...
const authorization = {
  ...
  evidenceFingerprint,
  evidence: {
    releaseGate: gate?.decision ?? "unknown",
    ...
    freshCriticalDiagnostics: critical.length,
    freshProductionFatals: freshFatals.length,
  },
  failures,
...
const md = [
  ...
  `- Main SHA: \`${authorization.currentMainSha ?? "—"}\``,
  ...
  "",
  "This contract does not deploy anything. It only authorizes promotion of the exact immutable Release Candidate above.",
  "",
  "Tehkné Solutions",
].join("\n");
...
if (baseUrl) {
  const event = {
    schemaVersion: 1,
    id: `deployment-authorization.${authorization.authorizationId}`,
    occurredAt: authorization.authorizedAt,
    kind: authorized ? "health" : "anomaly",
    name: authorized ? "deployment_authorized" : "deployment_authorization_rejected",
    level: authorized ? "info" : "error",
    sessionId: `deploy.${(candidate?.candidateSha ?? currentMainSha).slice(0, 12)}`,
    data: {
  ...
  try {
    await fetch(`${baseUrl}/v1/telemetry`, {
      ...
      body: JSON.stringify({ release: "deployment-authorization-contract", buildSha: candidate?.candidateSha ?? currentMainSha, events: [event] }),
    });
  } catch (error) {
    console.warn("deployment authorization telemetry publish failed", error instanceof Error ? error.message : error);
  }
}
